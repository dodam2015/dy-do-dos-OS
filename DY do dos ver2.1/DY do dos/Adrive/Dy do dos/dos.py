...
while cmd!='shutdown -s -lo- -li-'or cmd!='exit':
    ...
    if cmd=='get Adrive capacity;':
        ...
        # developer.py 파일이 존재하는지 확인
        if os.path.isfile(file_path):
            # 파일이 존재하면 실행
            subprocess.run(['python', file_path])
        else:
            print("")
    elif cmd=='beta downloader;':
        print('1.beta1')
        beta_sec=input('select version:')
        if beta_sec=='1' or beta_sec=='beta1':
            # 웹사이트 URL
            ...
            if response.status_code == 200:
                ...
                with open(file_path, 'w', encoding='utf-8') as file:
                    file.write(text)  # 전체 내용 저장

                print(f"beta1 version is downloaded.")
            else:
